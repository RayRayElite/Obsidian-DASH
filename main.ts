...
export default class DailyDashboardPlugin extends Plugin {
  ...
  private async writeManagedTextFile(path: string, content: string): Promise<void> {
  ...
  private async writeManagedMarkdownFile(path: string, content: string): Promise<void> {
  ...
  private markKanbanManagedWrite(path: string): void {
    const normalizedPath = normalizePath(path);
  ...
  private isKanbanBoardNotePath(path: string): boolean {
    const normalizedPath = normalizePath(path).toLowerCase();
    const normalizedFolder = normalizePath(this.data.settings.kanbanBoardNotesFolder).toLowerCase();
    return normalizedPath.startsWith(`${normalizedFolder}/`) && normalizedPath.endsWith(".md");
  }

  private shouldAutoSyncKanbanArtifact(path: string): boolean {
    const normalizedPath = normalizePath(path);
    ...
    return normalizedPath === normalizePath(this.data.settings.kanbanHubPath) || this.isKanbanBoardNotePath(normalizedPath);
  }

  private scheduleKanbanArtifactSync(path: string): void {
    this.pendingKanbanSyncPath = normalizePath(path);
    ...
    this.kanbanSyncDebounceId = window.setTimeout(() => {
      ...
      if (targetPath === normalizePath(this.data.settings.kanbanHubPath)) {
  ...
  async onload(): Promise<void> {
    ...
    this.addCommand({
      id: "apply-kanban-cleanup-migration",
      name: "Apply Kanban cleanup migration",
      callback: () => {
        void this.applyKanbanCleanupMigration(true);
      }
    });
  ...
  private reconcileUniqueLabelLaneMappingsToProjectSections(
  ...
  private async ensureSupportNoteExistsWithoutStatus(pathValue: string, renderTemplate: () => string): Promise<void> {
    const path = normalizePath(pathValue);
  ...
  private async writeKanbanCleanupPreviewFile(generatedAt: Date, preview: KanbanMigrationPreview): Promise<TFile> {
    return this.upsertMarkdownFile(this.getKanbanCleanupPreviewPath(), this.renderKanbanCleanupPreview({
      generatedAt,
      preview,
      boardNotePaths: this.getExistingKanbanBoardNotePaths()
    }));
  }

  private normalizeKanbanLaneDefinitions(value: unknown): KanbanLaneDefinition[] {
    if (!Array.isArray(value)) {
      return [];
    }

    return value
      .filter((entry): entry is Partial<KanbanLaneDefinition> => Boolean(entry) && typeof entry === "object")
      .map((entry) => {
      ...
      .filter((entry) => entry.laneKey.length > 0 && entry.label.length > 0);
  }

  private normalizeKanbanBoardTemplates(value: unknown): Record<string, KanbanBoardTemplate> {
    if (!value || typeof value !== "object") {
      return {};
    }

    const boardTemplates: Record<string, KanbanBoardTemplate> = {};
    Object.entries(value as Record<string, Partial<KanbanBoardTemplate>>).forEach(([templateId, entry]) => {
      if (!entry || typeof entry !== "object") {
        return;
      }

      const normalizedTemplateId = typeof entry.templateId === "string" && entry.templateId.trim().length > 0 ? entry.templateId.trim() : templateId.trim();
      if (!normalizedTemplateId) {
        return;
      }

      boardTemplates[normalizedTemplateId] = {
        templateId: normalizedTemplateId,
        name: typeof entry.name === "string" ? entry.name.trim() : normalizedTemplateId,
  ...
  private normalizeKanbanBoardConfigurations(value: unknown): Record<string, KanbanBoardConfiguration> {
    ...
    const boardConfigurations: Record<string, KanbanBoardConfiguration> = {};
    Object.entries(value as Record<string, Partial<KanbanBoardConfiguration>>).forEach(([projectName, entry]) => {
      if (!entry || typeof entry !== "object") {
        return;
      }

      const normalizedProjectName = typeof entry.projectName === "string" && entry.projectName.trim().length > 0 ? entry.projectName.trim() : projectName.trim();
      if (!normalizedProjectName) {
        return;
      }

      boardConfigurations[normalizedProjectName] = {
        projectName: normalizedProjectName,
  ...
  private normalizeKanbanRepairQueue(value: unknown): Record<string, KanbanRepairStateRecord> {
    if (!value || typeof value !== "object") {
      return {};
    }

    const repairQueue: Record<string, KanbanRepairStateRecord> = {};
    Object.entries(value as Record<string, Partial<KanbanRepairStateRecord>>).forEach(([repairId, entry]) => {
      if (!entry || typeof entry !== "object") {
        return;
      }

      const normalizedRepairId = typeof entry.repairId === "string" && entry.repairId.trim().length > 0 ? entry.repairId.trim() : repairId.trim();
      if (!normalizedRepairId) {
        return;
      }

      const reason = entry.reason === "ambiguous-match"
        || entry.reason === "conflict"
        || entry.reason === "orphaned-task"
        || entry.reason === "broken-source-line"
        ? entry.reason
        : "orphaned-task";

      repairQueue[normalizedRepairId] = {
        ...
        resolvedAt: typeof entry.resolvedAt === "string" ? entry.resolvedAt.trim() : ""
      };
    });
  ...
  private syncKanbanRepairQueueFromPreview(preview: KanbanMigrationPreview, updatedAt: string): number {
    let changed = 0;
    const activeRepairIds = new Set<string>();

    preview.brokenLegacyTaskLines.forEach((task) => {
      const repairId = task.taskId
        ? `legacy-broken-line:${task.taskId}`
        : `legacy-broken-line:${task.projectName}:${task.lineNumber}`;
    ...
    Object.keys(this.data.kanbanState.repairQueue).forEach((repairId) => {
      if (!repairId.startsWith("legacy-broken-line:")) {
        return;
      }
  ...
  private renderKanbanCleanupPreview(input: {
    ...
    return [
      ...
      "",
      "## Safe Cleanup Task List",
      ...safeCleanupTaskLines,
      ...(input.preview.tasksWithVisibleId.length > 80 ? ["", `- ${input.preview.tasksWithVisibleId.length - 80} additional task${input.preview.tasksWithVisibleId.length - 80 === 1 ? " is" : "s are"} omitted from this preview for readability.`] : []),
      "",
      ...
      "- Apply Kanban cleanup migration removes visible task ids only from checklist tasks that this preview marked as safe and then regenerates the current Kanban artifacts.",
  ...
  async previewKanbanCleanupMigration(openAfterGenerate: boolean): Promise<TFile | null> {
    ...
    if (openAfterGenerate) {
      await this.openFile(file);
      new Notice(`Kanban cleanup preview generated with ${preview.tasksWithVisibleId.length} checklist task${preview.tasksWithVisibleId.length === 1 ? "" : "s"} ready for visible-id cleanup${preview.brokenLegacyTaskLines.length > 0 ? `; ${preview.brokenLegacyTaskLines.length} broken line${preview.brokenLegacyTaskLines.length === 1 ? " still needs" : "s still need"} repair first` : "."}`);
    }

    return file;
  }

  async applyKanbanCleanupMigration(openAfterGenerate: boolean): Promise<TFile | null> {
    if (!this.data.kanbanState.lastCleanupPreviewAt) {
      if (openAfterGenerate) {
        new Notice("Run Preview Kanban cleanup migration before applying cleanup.");
      }
      return null;
    }
    ...
    let boardFiles: TFile[] = [];

    if (cleanup.removedVisibleTaskIds > 0) {
      await this.app.vault.modify(todoFile, cleanup.content);
      previewAfter = inspectMasterHubKanbanMigration(cleanup.content);
      boardFiles = await this.refreshKanbanBoardNotes(false);
      await this.refreshKanbanHub(false);
    }
    ...
    if (openAfterGenerate) {
      await this.openFile(previewFile);

      if (cleanup.removedVisibleTaskIds > 0) {
        new Notice(`Kanban cleanup removed ${cleanup.removedVisibleTaskIds} visible task id${cleanup.removedVisibleTaskIds === 1 ? "" : "s"} and regenerated the Hub plus ${boardFiles.length} board note${boardFiles.length === 1 ? "" : "s"}${cleanup.remainingBrokenLegacyTaskLines > 0 ? `; ${cleanup.remainingBrokenLegacyTaskLines} broken line${cleanup.remainingBrokenLegacyTaskLines === 1 ? " remains" : "s remain"} in the repair queue.` : "."}`);
      } else if (previewBefore.tasksWithVisibleId.length === 0) {
        new Notice(previewBefore.brokenLegacyTaskLines.length > 0
          ? `No checklist task ids needed cleanup. ${previewBefore.brokenLegacyTaskLines.length} broken legacy line${previewBefore.brokenLegacyTaskLines.length === 1 ? " is" : "s are"} still queued for repair.`
          : "No visible Kanban task ids remain to clean up.");
      } else {
        new Notice("Kanban cleanup preview changed, but no visible task ids were removed. Review the preview note before trying again.");
      }
    }
  ...
  async refreshKanbanBoardNotes(openAfterGenerate: boolean): Promise<TFile[]> {
    const source = await this.getKanbanSnapshotSource();
    if (!source) {
      if (openAfterGenerate) {
        new Notice("Master task hub not found. Set the path in plugin settings.");
  ...
  async refreshAllKanbanArtifacts(openAfterGenerate: boolean): Promise<void> {
    ...
    if (openAfterGenerate) {
      new Notice(`Refreshed Kanban Hub and ${boardFiles.length} board note${boardFiles.length === 1 ? "" : "s"}.`);
    }
  }
  ...
  async repairKanbanFoundations(showNotice: boolean): Promise<void> {
    ...
    if (showNotice) {
      new Notice(hubFile
        ? `Kanban foundations repaired, Kanban Hub refreshed, and ${boardFiles.length} board note${boardFiles.length === 1 ? "" : "s"} regenerated.`
        : "Kanban repair could not complete because the Master Task Hub is missing.");
    }
  }
  ...
  async syncKanbanBoardNoteToMasterTaskHub(boardPath: string, showNotice: boolean): Promise<void> {
    const todoFile = this.getMasterTodoFile();
    const boardFile = this.app.vault.getAbstractFileByPath(normalizePath(boardPath));
    if (!todoFile || !(boardFile instanceof TFile)) {
      if (showNotice) {
        new Notice("Master task hub or Kanban board note is missing.");
      }
      return;
    }
  ...
  async renameKanbanColumn(projectName: string, columnKey: string, nextLabel: string): Promise<boolean> {
    const normalizedProjectName = projectName.trim();
  ...
  private getDocumentationPageDefinitions(): DashboardDocumentationPageDefinition[] {
    return [
      ...
      {
        entry: {
          id: "docs-beta-install-support",
          title: "Beta Install and Support",
          description: "How to install the public beta, what the beta promise covers, and what to include in issue reports.",
          section: "Start here",
          path: `${DASH_DOCUMENTATION_FOLDER}/DASH Docs - Beta Install and Support.md`,
          keywords: ["beta", "install", "brat", "support", "issues", "releases"]
        },
        render: () => this.renderDocumentationBetaInstallSupportTemplate()
      },
      {
        entry: {
      ...
      {
        entry: {
          id: "docs-kanban-themes-templates",
          title: "Kanban Themes and Templates Guide",
          description: "How to customize board themes and templates safely, including the strict-JSON template limitation.",
          section: "Deep dives",
          path: `${DASH_DOCUMENTATION_FOLDER}/DASH Docs - Kanban Themes and Templates Guide.md`,
          keywords: ["kanban", "themes", "templates", "customization", "css", "json"]
        },
        render: () => this.renderDocumentationKanbanThemesTemplatesTemplate()
      },
      {
        entry: {
          id: "docs-ai-research",
  ...
  private renderDocumentationHomeTemplate(): string {
    return [
      ...
      "- [[DASH Docs - Beta Install and Support]]: BRAT install path, beta scope, and support expectations.",
      "",
      "### Core Workflows",
      ...
      "",
      "### Deep Dives",
      "- [[DASH Docs - Kanban Themes and Templates Guide]]: how to make safe custom board assets and what formats the loader accepts.",
  ...
  private renderDocumentationBetaInstallSupportTemplate(): string {
    return [
      "# DASH Docs - Beta Install and Support",
      "",
      ...
      "- native DASH Kanban core workflow",
      ...
      "- gamification",
      "",
      "## What Is Not A Current Promise",
      "These are possible future ideas, not current release commitments:",
      "- richer Trello-style Kanban extras such as comments, assignee images, non-image attachments, or percentage progress bars",
      "- nutrition-depth food tracking",
      "- voice activation",
      ...
      "",
      "## Where Feedback Goes",
      "For the public beta, GitHub Issues are the primary support and feedback channel.",
  ...
  private renderDocumentationDashboardGuideTemplate(): string {
    return [
      ...
      "",
      "## UI Customization",
      "The dashboard is meant to be rearranged. The first layout is only a starting point.",
      "",
      "### Reorder Cards By Drag And Drop",
      "- Drag a card by its header to move it earlier or later in the dashboard.",
      "- Pinned cards stay prioritized near the top, so pin only the few cards you want to stay important.",
      "- Hidden cards are not deleted. They are just removed from the current view until you show them again.",
      "",
      "### Use The Layout Editor For Deeper Changes",
      "Open the layout editor from the sliders icon in the hero footer or with Alt+Shift+L.",
      "- reorder cards more deliberately",
      "- pin cards that should stay near the top",
      "- hide cards you do not want on screen",
      "- change card width between default, half width, and full width",
      "- reset the dashboard back to the default layout",
      "",
      "### View Modes Matter",
      "- Mobile is for tighter vertical reading.",
      "- Compact is the default balanced desktop mode.",
      "- Widescreen is better when you want more cards visible at once.",
      "Cycle view modes from the hero footer or with Alt+Shift+V.",
      "",
      "## Hero Header And Utility Controls",
      "The hero area is the dashboard control rail, not just a banner.",
      "",
      "### Main Hero Buttons",
      "- New project: create a project and its related structure.",
      "- Add to project: quick-capture a task straight into a project and active section.",
      "- Review mode: open the structured project review workflow.",
      "- Repair day: fix logical-day issues when tracking drifted or something was left open incorrectly.",
      "- Notifications: open reminders, warnings, and system notices that need attention.",
      "",
      ...
      "- warning triangle icon: generate the recurring friction patterns note",
      "- bar chart icon: generate the weekly report",
      ...
      "- archived: how many tasks have already been completed and archived today",
      "- stale: how many projects currently look stale",
      "- mood and energy: the latest state snapshot",
      "- hidden: how many cards are currently hidden from the layout",
      "",
      "## Notifications And Undo",
      "- Use notifications when the badge count is non-zero or the bell looks alert-colored.",
      "- Notifications can surface calendar reminders, system setup issues, task pressure, and logical-day warnings.",
      "- Some notifications jump straight into the right repair or setup flow.",
      "- Destructive dashboard actions also create a temporary undo banner.",
      "",
      "## Session Deck Customization",
      "- reorder trackers when the default button order does not match your workflow",
      "- rename trackers to fit your own language",
      "- hide trackers you do not use often",
      "- change tracker colors when you want clearer visual separation",
      "",
      "## Layout Tips",
      ...
      "",
      "## Logical Day Rule",
      "DASH uses a logical day instead of assuming your real day resets at midnight.",
      ...
      "If you stay up past midnight, DASH will keep treating that time as part of the same day until you end it.",
      "",
      "## Where History Lives",
      "- daily logs for factual recall",
      "- weekly and monthly reports for interpretation",
      "- timeline search for retrieval across sessions, tasks, events, and text logs",
      "",
      "## Related Guides",
      "- [[DASH Docs - Projects and Kanban Guide]]",
  ...
  private renderDocumentationProjectsKanbanTemplate(): string {
    return [
      ...
      "Board settings are also where you should look for sticky headers, lane-category visibility, theme choice, and board height.",
      "",
      "## Kanban Tips",
      ...
      "- [[DASH Docs - Quick Start]]",
      "- [[DASH Docs - Daily Dashboard Guide]]",
      "- [[DASH Docs - Kanban Themes and Templates Guide]]",
      "- [[DASH Docs - FAQ]]",
      ""
    ].join("\n");
  }

  private renderDocumentationKanbanThemesTemplatesTemplate(): string {
    return [
      "# DASH Docs - Kanban Themes and Templates Guide",
      "",
      "Use this guide when you want to customize DASH Kanban with your own themes or templates.",
      "",
      "## Where The Files Live",
      "- `themes/` holds board themes.",
      "- `templates/` holds board templates.",
      "- Run the reload command after editing either asset type so DASH picks up the changes.",
      "",
      "## Important Format Difference",
      "- Theme files are CSS, so normal comments are safe.",
      "- Template files are strict JSON, so inline comments are not safe unless the loader is changed to support JSONC.",
      "- That is why template field explanations live in this guide instead of inside the shipped JSON files.",
      "",
      "## Theme Metadata Header",
      "Theme files can start with a JSON metadata block inside a CSS comment. DASH reads that block for the theme label, description, and preview colors.",
      "",
      "## Template Fields",
      "- `templateId`: stable unique id.",
      "- `name`: user-facing template label.",
      "- `description`: short explanation of what the board layout is for.",
      "- `builtIn`: whether the template ships with DASH.",
      "- `laneDefinitions`: board lane definitions used to map workflow and grouping.",
      "",
      "## Lane Definition Fields",
      "- `laneKey`: stable lane id.",
      "- `label`: visible lane title.",
      "- `helperText`: small supporting description.",
      "- `columnKey`: shared column id for grouped or matrix layouts.",
      "- `categoryKey`: shared row or swimlane id.",
      "- `categoryLabel`: visible group label.",
      "- `categorySubtitle`: optional group subtitle.",
      "- `categoryColor`: group identity color.",
      "- `categoryTag`: optional stable tag-like identifier.",
      "- `ruleType`: how the lane maps to the workflow.",
      "- `mappedSections`: Master Task Hub sections that feed the lane.",
      "- `done`: whether the lane counts as complete work.",
      "",
      "## Safe Customization Workflow",
      "1. Copy a built-in file into a new custom file.",
      "2. Rename the theme `id` or template `templateId`.",
      ...
      "",
      "## Recommended Built-In References",
      "- `dark.css`: minimal token-first dark theme reference.",
      "- `light.css`: full-override theme reference.",
      "- `execution-default.json`: straightforward execution board reference.",
      "- `support-swimlanes.json`: grouped swimlane reference.",
      "",
      "## Related Guides",
      "- [[DASH Docs - Projects and Kanban Guide]]",
      "- [[DASH Docs - FAQ]]",
  ...
  private async reloadDailyLogFile(file: TFile): Promise<void> {
    ...
    try {
      ...
      new Notice(`Could not reload daily log ${file.basename}. Check the file for malformed content.`);
    }
  }
