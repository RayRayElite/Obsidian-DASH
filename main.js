...
var CORE_SESSION_TRACKER_OPTIONS = ["work", "nap", "relax", "break", "poop"];
var DEFAULT_SESSION_TRACKERS = [
...
function renderTodoSnapshotForAi(snapshot) {
  ...
  const dueLines = snapshot.overdueTasks.slice(0, 6).map((item) => `- ${item.project}: ${formatTaskSummary(item.task)}`);
  const blockedLines = snapshot.blockedTasks.slice(0, 6).map((item) => `- ${item.project}: ${formatTaskSummary(item.task)}`);
  return [
...
function renderFinanceMonthlyReview(input) {
  var _a, _b, _c;
  const monthLabel = formatMonthLabel(input.monthKey);
  const visibleSubscriptions = input.financeData.subscriptions.filter((subscription) => subscription.status !== "archived");
  const recurringSubscriptions = visibleSubscriptions.filter((subscription) => subscription.kind === "recurring" && subscription.status !== "canceled");
  ...
  const annualRenewals = recurringSubscriptions.filter((subscription) => subscription.intervalMonths >= 12).sort((left, right) => left.renewalDate.localeCompare(right.renewalDate) || left.name.localeCompare(right.name));
  const dueSoonSubscriptions = activeSubscriptions.map((subscription) => ({ subscription, daysUntilRenewal: getDaysUntilDate(subscription.renewalDate, input.generatedAt) })).filter((item) => item.daysUntilRenewal !== null && item.daysUntilRenewal >= 0 && item.daysUntilRenewal <= 45).sort((left, right) => {
    var _a2, _b2;
    return ((_a2 = left.daysUntilRenewal) != null ? _a2 : 999) - ((_b2 = right.daysUntilRenewal) != null ? _b2 : 999);
  });
  const monthlyRecurringTotal = recurringSubscriptions.reduce((sum, subscription) => sum + getSubscriptionMonthlyEquivalent(subscription), 0);
  const yearlyRecurringTotal = recurringSubscriptions.reduce((sum, subscription) => sum + getSubscriptionAnnualizedCost(subscription), 0);
  const totalBudgetTarget = input.financeData.budgetCategories.reduce((sum, category) => sum + category.monthlyTarget, 0);
  const budgetPressureRows = input.financeData.budgetCategories.map((category) => {
  ...
  const categoryConcentration = budgetPressureRows.filter((row) => row.committedRecurring > 0).sort((left, right) => right.committedRecurring - left.committedRecurring).slice(0, 3);
  const creepSignals = [
  ...
  return [
    ...
    `- Main blocker: ${budgetPressureRows.find((row) => row.gap < 0) ? `${(_a = budgetPressureRows.find((row) => row.gap < 0)) == null ? void 0 : _a.category.label} is already over target from recurring commitments.` : dueSoonSubscriptions[0] ? `${dueSoonSubscriptions[0].subscription.name} renews in ${dueSoonSubscriptions[0].daysUntilRenewal} days and needs a keep/cancel decision.` : "No immediate finance blocker stands out from the current tracked data."}`,
    ...
    "",
    "## Renewal Insights",
    ...dueSoonSubscriptions.length > 0 ? dueSoonSubscriptions.map(({ subscription, daysUntilRenewal }) => `- ${subscription.name}: renews in ${daysUntilRenewal} day${daysUntilRenewal === 1 ? "" : "s"} \u2022 ${formatFinanceAmount(subscription.cost, subscription.currency)} \u2022 ${formatSubscriptionCycle(subscription)}${subscription.cancelUrl ? ` \u2022 cancel ${subscription.cancelUrl}` : ""}`) : ["- No subscription renewals are due in the next 45 days."],
    "",
    "## Annual Renewal Exposure",
    ...annualRenewals.length > 0 ? annualRenewals.map((subscription) => `- ${subscription.name}: ${formatFinanceAmount(subscription.cost, subscription.currency)} yearly-equivalent charge \u2022 next renewal ${subscription.renewalDate || "unknown"}`) : ["- No annual renewals are currently tracked."],
    "",
    "## Budget Pressure Review",
    ...budgetPressureRows.length > 0 ? budgetPressureRows.map((row) => `- ${row.category.label}: target ${formatFinanceAmount(row.category.monthlyTarget, "USD")} \u2022 committed recurring ${formatFinanceAmount(row.committedRecurring, "USD")} \u2022 headroom ${formatFinanceAmount(row.gap, "USD")} \u2022 ${row.linkedCount} linked recurring item${row.linkedCount === 1 ? "" : "s"}`) : ["- No budget categories are configured yet."],
    "",
    "## Payment Method Concentration",
    ...paymentMethodBreakdown.size > 0 ? [...paymentMethodBreakdown.entries()].sort((left, right) => right[1] - left[1]).map(([method, count]) => `- ${method}: ${count} active subscription${count === 1 ? "" : "s"}`) : ["- No payment methods are recorded yet."],
    "",
    "## Review Actions",
    `- [ ] Review ${((_b = dueSoonSubscriptions[0]) == null ? void 0 : _b.subscription.name) || "the next due renewal"} and decide keep, cancel, or downgrade.`,
    `- [ ] Confirm ${((_c = categoryConcentration[0]) == null ? void 0 : _c.category.label) || "the highest-cost category"} still deserves its current recurring load.`,
...
function renderKanbanProjectBoard(project) {
  ...
  const summaryMetrics = [
    ...
    `Next ${(_d = (_c = laneTasks.get("Next")) == null ? void 0 : _c.length) != null ? _d : 0}`,
    `Waiting ${(_f = (_e = laneTasks.get("Waiting")) == null ? void 0 : _e.length) != null ? _f : 0}`,
    `Done ${(_h = (_g = laneTasks.get("Done")) == null ? void 0 : _g.length) != null ? _h : 0}`,
...
function renderKanbanReviewHelpers(snapshot) {
  ...
  const blockedTaskLines = snapshot.blockedTasks.slice(0, 8).map(({ project, task }) => {
    ...
    const projectLabel = (projectSummary == null ? void 0 : projectSummary.noteLinks[0]) ? createWikiLink(projectSummary.noteLinks[0], projectSummary.name) : project;
    const details = [
      task.blockedReason ? `blocked ${task.blockedReason}` : "",
      task.unblockDate ? `unblock ${task.unblockDate}` : "",
      (projectSummary == null ? void 0 : projectSummary.waitingOn.trim()) && projectSummary.waitingOn.trim().toLowerCase() !== "none" ? `waiting on ${projectSummary.waitingOn.trim()}` : "",
      (projectSummary == null ? void 0 : projectSummary.nextAction.trim()) ? `next ${projectSummary.nextAction.trim()}` : ""
    ].filter((value) => value.length > 0);
...
function selectProjectNextAction(input) {
  ...
  if (actionableTask) {
    return actionableTask.minimumStep.trim().length > 0 ? `${actionableTask.minimumStep} (minimum step for ${actionableTask.text})` : actionableTask.text;
  }
        ...
        reminderItems.forEach(({ reminder, id }) => {
          ...
          copy.createEl("span", {
            ...
            text: this.getFocusDisplayMeta({
              ...
              calendarNotes: [this.plugin.renderCalendarProjectLink(reminder.projectName, reminder.projectNotePath), reminder.notes].filter((value) => value.trim().length > 0).join(" \u2022 "),
              repeatCadence: reminder.repeatCadence,
              warningLevel: reminder.warningLevel
            })
          });
          const reminderNotes = [this.plugin.renderCalendarProjectLink(reminder.projectName, reminder.projectNotePath), reminder.notes].filter((value) => value.trim().length > 0).join(" \u2022 ");
          if (reminderNotes) {
            copy.createEl("span", {
              cls: "daily-dashboard-row-meta",
              text: reminderNotes
          ...
          createButton(controls, "Calendar", async () => {
            var _a2, _b2;
            new CalendarEventModal(this.app, this.plugin, (_a2 = reminder.date) != null ? _a2 : todayEntry.date, (_b2 = reminder.id) != null ? _b2 : null).open();
          }, false, "calendar-days");
        });
      }
...
var _DailyDashboardPlugin = class _DailyDashboardPlugin extends import_obsidian4.Plugin {
  ...
  getActivitySessionTrackers() {
    return this.getSessionTrackers().filter((tracker) => !CORE_SESSION_TRACKER_OPTIONS.includes(tracker.id));
  }
  getVisibleActivitySessionTrackers() {
    return this.getVisibleSessionTrackers().filter((tracker) => !CORE_SESSION_TRACKER_OPTIONS.includes(tracker.id));
  }
  ...
  getSuggestedTop3Candidates(todoSnapshot, calendarSnapshot) {
    ...
    ((_d = todoSnapshot == null ? void 0 : todoSnapshot.staleProjects) != null ? _d : []).slice(0, 3).forEach((project) => {
      ...
      pushCandidate({
        ...
        estimateMinutes: null,
        reason: `${project.name} has been stale for ${(_d2 = project.staleDays) != null ? _d2 : 0}d${task.trigger ? ` \u2022 trigger ${task.trigger}` : ""}`,
        source: "stale"
      });
    });
  ...
  async reorderHabitDefinitions(fromIndex, toIndex) {
    const definitions = [...this.getHabitDefinitions()];
  ...
  async askResearchQuestionAndWriteWikiNotes(input) {
    ...
    try {
      ...
      if (!this.getResolvedAiApiKey()) {
        new import_obsidian4.Notice(`Research question seed note created. ${this.getAiConfigurationMessage()}`);
        return;
      }
      ...
      if (generatedFiles.length > 0) {
        new import_obsidian4.Notice(`Research question workflow generated ${generatedFiles.length} wiki note${generatedFiles.length === 1 ? "" : "s"}.`);
        return;
      }
      new import_obsidian4.Notice("Research question seed note was created, but no wiki notes were generated.");
    } catch (error) {
      const message = error instanceof Error ? error.message : `${error}`;
      new import_obsidian4.Notice(`Research question workflow failed: ${message}`);
    }
  }
...
${context}`
      ...
      const file = await this.createKnowledgeBaseSlideDeckNote({
        folder: `${normalizeFolderPath2(this.data.settings.knowledgeBaseOutputsFolder)}/Slides`,
        fileLabel,
      ...
      this.refreshDashboardViews();
      await this.openFile(file);
      new import_obsidian4.Notice("Research Marp slide deck generated.");
    } catch (error) {
      const message = error instanceof Error ? error.message : `${error}`;
  ...
  async promoteActiveResearchOutputToConceptNote() {
  ...
  async promoteFollowUpQuestionsFromActiveResearchNote() {
    var _a;
    const activeFile = this.getActiveMarkdownFile("Open a research note before promoting its follow-up questions into the index.");
    if (!activeFile) {
      return;
    }
    const questions = this.extractSectionBulletLines(await this.app.vault.read(activeFile), "Follow-Up Questions").map((item) => item.replace(/^\[[ xX]\]\s+/, "").trim()).filter((item) => item.length > 0);
    if (questions.length === 0) {
      new import_obsidian4.Notice("No follow-up questions section was found in the active research note.");
      return;
    }
    const openQuestionsPath = (_a = this.getKnowledgeBaseStarterNoteDefinitions().find((note) => note.key === "questions")) == null ? void 0 : _a.path;
    if (!openQuestionsPath) {
      new import_obsidian4.Notice("Open Questions note path is not configured.");
      return;
    }
    ...
    new import_obsidian4.Notice(updated > 0 ? `Added ${updated} follow-up question${updated === 1 ? "" : "s"} to Open Questions.` : "No new follow-up questions needed to be added.");
  ...
  async generateCompiledResearchRetrievalTuningNote() {
    const file = await this.createCompiledResearchRetrievalTuningNote();
    await this.openFile(file);
    new import_obsidian4.Notice("Compiled research retrieval tuning note generated.");
  }
  ...
  async createCompiledResearchRetrievalTuningNote() {
    const generatedAt = /* @__PURE__ */ new Date();
    ...
    const rawIndexed = indexedFolders.includes(normalizeFolderPath2(this.data.settings.knowledgeBaseRawFolder));
    const knowledgeBaseEntries = Object.values(this.data.noteIndex.entries).filter((entry) => recommendedFolders.some((folder) => entry.path === folder || entry.path.startsWith(`${folder}/`)));
    ...
    const guidance = [
      noteCount < 25 ? "Current wiki volume is still small enough that folder-scoped keyword retrieval should stay effective without extra tuning." : noteCount < 75 ? "Wiki volume is entering the range where chunk quality and note naming matter more than adding complex retrieval layers." : "Wiki volume is large enough that embeddings or stricter retrieval scoping may now be worth the added cost.",
      rawIndexed ? `Raw sources are currently included in AI indexed folders. Consider removing ${this.data.settings.knowledgeBaseRawFolder} unless direct raw-source retrieval is worth the noise.` : "Raw-source indexing is still optional, which keeps retrieval cleaner for synthesized answers.",
      this.data.settings.aiEmbeddingsEnabled ? `Embeddings are enabled with ${embeddedChunkCount} embedded knowledge-base chunk${embeddedChunkCount === 1 ? "" : "s"}. Reassess only if semantic matches still feel noisy or shallow.` : noteCount >= 75 ? "Embeddings are currently off. At this note volume, enabling them may improve concept-level retrieval across differently worded notes." : "Embeddings are currently off, which is still a reasonable default at the current wiki size.",
      missingRecommendedFolders.length === 0 ? "All recommended compiled wiki folders are in AI indexed folders." : `Recommended compiled folders missing from AI indexed folders: ${missingRecommendedFolders.join(", ")}.`
    ];
    const filePath = this.getAvailableMarkdownPath(`${normalizeFolderPath2(this.data.settings.knowledgeBaseOutputsFolder)}/Health Checks/${formatDateKey(generatedAt)} Retrieval Tuning.md`);
    const content = [
...
${context}`, resolvedModel);
  ...
  async createKnowledgeBaseSlideDeckNote(input) {
    const timestamp = formatFileTimestamp(/* @__PURE__ */ new Date());
    const folder = normalizeFolderPath2(input.folder);
  ...
  async appendUniqueBulletsToSection(file, heading, bullets) {
    const content = await this.app.vault.read(file);
    const lines = content.split(/\r?\n/);
    const targetHeading = heading.trim().toLowerCase();
    const normalizedBullets = bullets.map((bullet) => bullet.trim()).filter((bullet, index, items) => bullet.length > 0 && items.indexOf(bullet) === index);
    ...
    const existingBullets = new Set(lines.slice(headingIndex + 1, sectionEndIndex).map((line) => {
      var _a, _b, _c;
  ...
  async generateMonthlyFinanceReview(openAfterGenerate) {
    const today = /* @__PURE__ */ new Date();
    const monthKey = formatDateKey(today).slice(0, 7);
    ...
    if (openAfterGenerate) {
      await this.openFile(file);
      new import_obsidian4.Notice("Monthly finance review generated.");
    }
    return file;
  }
