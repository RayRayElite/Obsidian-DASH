...
The current Kanban slice now has two surfaces. The older note-first path still exposes `Refresh Kanban Hub`, `Create or refresh Kanban board notes`, `Refresh all Kanban artifacts`, `Repair Kanban foundations and refresh hub`, `Preview Kanban cleanup migration`, `Apply Kanban cleanup migration`, `Repair Kanban links`, `Sync Kanban Hub to Master Task Hub`, `Kanban quick add task`, and `Edit Kanban task`. In addition, the new `Open DASH Kanban board` command and ribbon icon open a dedicated custom board view with template-driven lanes, both `All Projects` and `Single Project` modes, search plus persisted quick filters for attention-blocked-due work, an in-view repair banner when the repair queue is non-empty, richer card metadata, a card detail panel, per-project board settings, direct complete and delete actions, and drag-and-drop lane moves that persist back into the Master Task Hub. Cross-project drag now transfers the same task into the target project while preserving hidden registry identity, and dropping into a done lane completes and archives the task through the current Master Task Hub flow.
