...
The current Kanban slice now has two surfaces. The older note-first path still exposes `Refresh Kanban Hub`, `Create or refresh Kanban board notes`, `Refresh all Kanban artifacts`, `Repair Kanban foundations and refresh hub`, `Preview Kanban cleanup migration`, `Apply Kanban cleanup migration`, `Repair Kanban links`, `Prune stale Kanban registry entries`, `Sync Kanban Hub to Master Task Hub`, `Kanban quick add task`, and `Edit Kanban task`. In addition, the new `Open DASH Kanban board` command and ribbon icon open a dedicated custom board view with template-driven lanes, both `All Projects` and `Single Project` modes, search plus persisted quick filters for attention-blocked-due work, a density toggle, an in-view repair banner when the repair queue is non-empty, richer card metadata, a card detail panel with inline quick edit controls, per-project board settings, direct complete and delete actions, drag-and-drop lane moves that persist back into the Master Task Hub, screenshot-style swimlane category bands, per-project resizable board heights with lane-level scrolling, and clickable project headers that collapse boards in the all-projects workspace. Built-in templates now also include screenshot-inspired `Support Swimlanes`, `Bugs And Features`, and `Content Campaign` layouts so those grouped board patterns can be reused directly.
