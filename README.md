...
The Kanban foundation slice now also adds stable `[task-id: ...]` annotations to active hub checklist items when needed and exposes `Refresh Kanban Hub`, `Repair Kanban foundations and refresh hub`, `Sync Kanban Hub to Master Task Hub`, `Kanban quick add task`, and `Edit Kanban task`. Those commands write a generated board note at the configured Kanban Hub path, let lane moves made in that note persist back into the Master Task Hub without replacing the hub as the canonical source of truth, surface blocked-task and stale-project review helpers directly inside the generated board note, add board-summary and filter sections for faster navigation, and give the command palette explicit flows for adding or rewriting Kanban tasks without editing raw hub markdown by hand.
