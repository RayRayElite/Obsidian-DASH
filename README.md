...
The current Kanban slice now has two surfaces. The older note-first path still exposes `Refresh Kanban Hub`, `Create or refresh Kanban board notes`, `Refresh all Kanban artifacts`, `Repair Kanban foundations and refresh hub`, `Preview Kanban cleanup migration`, `Apply Kanban cleanup migration`, `Repair Kanban links`, `Prune stale Kanban registry entries`, `Sync Kanban Hub to Master Task Hub`, `Kanban quick add task`, and `Edit Kanban task`. In addition, the new `Open DASH Kanban board` command and ribbon icon open a dedicated custom board view with template-driven lanes, both `All Projects` and `Single Project` modes, search plus persisted quick filters for attention-blocked-due work, a slimmer collapsible header, an in-view repair banner when the repair queue is non-empty, stronger per-project themes, direct complete and delete actions, compact inline quick-add composers in lane headers, click-to-edit cards with in-card metadata editing for task text, lane, priority, due date, blocked reason, effort, and context, inline lane-title rename controls, compact footer-aligned card actions, drag-and-drop lane moves that persist back into the Master Task Hub, same-lane manual reordering that persists within a priority band across refreshes, optional swimlane category bands, per-project resizable board heights with lane-level scrolling, clickable project headers that collapse boards in the all-projects workspace, honest `Custom` template labeling when a board diverges from its base preset, shared-column matrix rendering for grouped templates that need aligned row sections instead of stacked lane strips, category-band names that round-trip through project metadata using stable `categoryKey=Label` pairs, and safe one-to-one lane-label renames that can rewrite the matching project section heading in the Master Task Hub when that lane is still mirroring a single hub section directly. If that rename would collide with an existing hub section, DASH now leaves the hub unchanged and surfaces a manual-review notice instead of merging headings silently. Built-in templates now include screenshot-inspired `Support Swimlanes`, `Bugs And Features`, and `Simple` layouts so grouped board patterns can reuse one column set across multiple row groups, while standard boards can still keep swimlane categories off entirely.
The current Kanban slice now has two surfaces. The older note-first path still exposes `Refresh Kanban Hub`, `Create or refresh Kanban board notes`, `Refresh all Kanban artifacts`, `Repair Kanban foundations and refresh hub`, `Preview Kanban cleanup migration`, `Apply Kanban cleanup migration`, `Repair Kanban links`, `Prune stale Kanban registry entries`, `Sync Kanban Hub to Master Task Hub`, `Kanban quick add task`, and `Edit Kanban task`. In addition, the new `Open DASH Kanban board` command and ribbon icon open a dedicated custom board view with template-driven lanes, both `All Projects` and `Single Project` modes, search plus persisted quick filters for attention-blocked-due work, a truly compact collapsed header that keeps only the title, status chips, core actions, and arrow toggle, an in-view repair banner when the repair queue is non-empty, stronger per-project themes, direct complete and delete actions, compact inline quick-add composers in lane headers, click-to-edit cards with a smaller in-card text editor, footer-anchored priority and due or effort popovers, due date plus effort surfaced in the card footer, inline lane-title rename controls, compact footer-aligned card actions, drag-and-drop lane moves that persist back into the Master Task Hub, same-lane manual reordering that persists within a priority band across refreshes, optional swimlane category bands, per-project resizable board heights with lane-level scrolling, clickable project headers that collapse boards in the all-projects workspace, honest `Custom` template labeling when a board diverges from its base preset, shared-column matrix rendering for grouped templates that need aligned row sections instead of stacked lane strips, category-band names that round-trip through project metadata using stable `categoryKey=Label` pairs, and safe one-to-one lane-label renames that can rewrite the matching project section heading in the Master Task Hub when that lane is still mirroring a single hub section directly. If that rename would collide with an existing hub section, DASH now leaves the hub unchanged and surfaces a manual-review notice instead of merging headings silently. Built-in templates now include screenshot-inspired `Support Swimlanes`, `Bugs And Features`, and `Simple` layouts so grouped board patterns can reuse one column set across multiple row groups, while standard boards can still keep swimlane categories off entirely.
