...
The Kanban foundation slice now also adds stable `[task-id: ...]` annotations to active hub checklist items when needed and exposes `Refresh Kanban Hub`, `Repair Kanban foundations and refresh hub`, plus `Sync Kanban Hub to Master Task Hub`. Those commands write a generated board note at the configured Kanban Hub path, let lane moves made in that note persist back into the Master Task Hub without replacing the hub as the canonical source of truth, and surface blocked-task and stale-project review helpers directly inside the generated board note.
